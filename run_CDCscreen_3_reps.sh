...
perl -alne 'next if $F[0]=~/#/; next if $F[0] eq "Geneid"; $rpk=0; $rpk=$F[2] if $F[2]>0; $totalrpk += $rpk; push @gene,$F[0];push @generpk,$rpk; END{ $scale=$totalrpk/100000; for($i=0;$i<=$#gene;$i++){$,="\t";print $gene[$i],$generpk[$i]/$scale+1;} }' 02_gRNA_reads.txt > 03_gRNA_D0_rep3.txt
...
perl -alne 'next if $F[0]=~/#/; next if $F[0] eq "Geneid"; $rpk=0; $rpk=$F[4] if $F[4]>0; $totalrpk += $rpk; push @gene,$F[0];push @generpk,$rpk; END{ $scale=$totalrpk/100000; for($i=0;$i<=$#gene;$i++){$,="\t";print $gene[$i],$generpk[$i]/$scale+1;} }' 02_gRNA_reads.txt > 03_gRNA_D30_rep3.txt
